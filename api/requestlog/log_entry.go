...
func (r *responseStats) WriteHeader (statusCode int) {
	//get headersize then write statuscode
	//update code
	if r.code != 0{
		return
	}
	r.hsize = headerSize(r.w.Header()) //ResponseWriter's header
...
func (r *responseStats) size() (hdr, body int64) {
	...
	//use headersize from time WriteHeader was called.
	//Header map can be mutated after call to add HTTP Trailers, which we don't want to count
